...
for (ii in 2:3){
  for (jj in 2:3){
    ...
    death_df_both <- c(death_df_both,round(sum(dfs[[l]]$Inc_pred)))
...
death_df_both <- as.data.frame(death_df_both)
colnames(death_df_both) <- c("y")
death_df_both$x <- c(#"No Change",
...
color_val_both <- c( #"red",
               "skyblue2",  "orange",
               "#b3ff00", "#096e02")
names(color_val_both) <- death_df_both$x
